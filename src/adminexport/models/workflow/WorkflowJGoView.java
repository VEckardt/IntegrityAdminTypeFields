...
/**
 * A view that shows the relationships in a graphical format, using the JGo tools.
 */
public class WorkflowJGoView extends JGoViewEx implements JGoViewListener, ComponentListener
{
	private static final long serialVersionUID = 82342456L;
	public static JGoPen normalPen = new JGoPen(JGoPen.SOLID, 3, new Color(129, 152, 178));
     ...
     */
    public WorkflowJGoView(JGoDocument doc)
    {
        super(doc);
    ...
    /**
     * Over-rides libjava.JGoViewEx.
     * Fixes "Zoom to Fit" problems. Somehow there were no "Zoom to Fit"
     * problems in HistoryGo, which also extends JGoViewEx. Whatever.
     * Zooming and legend have been very annoying.
     */
    ...
    /**
     * Capture various events that happen in this view, and
     * fire WorkflowEvents to interested components.
     * 
     ...
     */
    public WorkflowJGoModel getDoc()
    {
        return (WorkflowJGoModel)getDocument();
    }
    ...
    /**
     * Runs the layout algorithm. The direction of the layout
     * coincides with the direction on the previous invocation.
     */
    ...
    /**
     * Override the preferred size.  We are always within a JSplitPane
     * which has priority for us.  If we return our actual document size,
     * which is the default, then the JSplitPane will give us priority
     * over the other component, which it will shrink down to its minimum
     * size.  Sine we never exist except in the component which will grow
     * us in preference, the preferred size will never actually get used
     * to size us.
     */
    ...
    /**
     * Over-ride for JGoView in order to fix the jpeg size when producing the image.
     */
    public java.awt.Dimension getPrintDocumentSize()
    {
        ...
        for (JGoListPosition pos = doc.getFirstObjectPos(); pos != null; pos = doc.getNextObjectPosAtTop(pos))
        {
            ...
            if(obj instanceof WorkflowJGoLink)
            {
            	WorkflowJGoLink link = (WorkflowJGoLink)obj;
